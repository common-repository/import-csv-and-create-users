...
function csvuserimport_menu() {	
	add_submenu_page( 'users.php', 'CSV User Import', 'Import from CSV', 'manage_options', 'csv-user-import', 'csvuserimport_page1');	
}
...
'aaaaaceeeeiiiinooooouuuuyyAAAAACEEEEIIIINOOOOOUUUUY');
	...
	wp_enqueue_script('user_import',plugins_url('/script.js', __FILE__),false,false,false);
	global $wpdb;
		...
		if (is_array($arr_rows)) {
			foreach ($arr_rows as $row) {
				...
				$arr_user = array( 	'user_login' => $username,
									...
									/// to prevent multiple users with same email and username
if( !email_exists( $user_email ) or !username_exists( $username )) {
	 ...
	 /// will add outgoing email in next version
	// $headers .= 'From: no-reply <no-reply@website.com>' . "\r\n";
		wp_mail($user_email, $mail_title, $message,$headers);
	$sending_to = "Sending email to :".$user_nicename." - Login : ".$username." and password :".$password."<br>";	
	$log_message = $log_message.$sending_to;		
				}
...
<p>
Please select the CSV file you want to import below. IMPORTANT : The CSV file should be in the following format: firstname,lastname,email address
</p>

<form action="users.php?page=csv-user-import" method="post" enctype="multipart/form-data">
<p><input type="file" name="csv_file" /></p>
<p>What role should be assigned to imported users? <select name = "role"><?wp_dropdown_roles('subscriber')?><
/select></p>
...
**login** = user login - **password** = user password - **siteurl** = current site's address - user nice name = **usernicename**
</div>
<p>
...
<p>Note : Password will be generated randomly and username = firstnamelastname</p>

<p style="color: red">
(Please make sure you back up your database before proceeding! or just go ahead it should work :))
</p>
</div>
<p><?=$log_message; ?></p>
<?php
